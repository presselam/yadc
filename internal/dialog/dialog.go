...
var (
	KeyDismiss = key.NewBinding(key.WithKeys(" ", "esc", "enter"))
	KeyRight   = key.NewBinding(key.WithKeys("right", "l"))
	KeyLeft    = key.NewBinding(key.WithKeys("left", "h"))
)

func NewDialog(title string, message string, buttons ...string) Model {
	m := Model{
		title:        title,
		message:      message,
		buttons:      buttons,
...
func (m Model) Confirmed() bool { return m.confirmation }
